...
public class OrganizationController {
    ...
    public ResponseEntity<OrganizationModel> search(@PathVariable(name = "id") UUID id) {
        return ResponseEntity.status(HttpStatus.OK).body(organizationModelMapper.toModel(organizationService.findById(id)));
    }

    @GetMapping("/filter")
    public ResponseEntity<Page<?>> filter(String name,
                                           @PageableDefault(page = 0, size = 10) Pageable pageable) {
        return ResponseEntity.status (HttpStatus.OK).body(organizationService.filter(name, pageable));
    }

    @PostMapping("/registry")
    public ResponseEntity<OrganizationModel> registry(@RequestBody @Valid OrganizationModelInput organizationModelInput) {
    ...
    public ResponseEntity<OrganizationModel> activateDeactivate(@RequestBody OrganizationActiveModelInput departamentoActiveModelInput,
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(
                organizationModelMapper.toModel(
                        organizationService.activeOrDesactived(id, departamentoActiveModelInput.isActive())));
    }
